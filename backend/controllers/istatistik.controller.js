...
exports.getGenelIstatistikler = async (req, res) => {
    try {
        const queryStr = `
            SELECT
                b.bolum_adi,
                ...
                ROUND(
                    IFNULL(SUM(CASE WHEN r.durum = 'tamamlandi' THEN 1 ELSE 0 END) / NULLIF(COUNT(r.id), 0) * 100, 0), 2
...
exports.getDoktorYuku = async (req, res) => {
    try {
        const queryStr = `
            SELECT
                d.id AS doktor_id,
            ...
            WHERE r.randevu_tarihi >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) OR r.randevu_tarihi IS NULL
...
exports.getAktifHastalar = async (req, res) => {
    try {
        const queryStr = `
            SELECT
                h.id AS hasta_id,
...
exports.getMusaitDoktorlar = async (req, res) => {
    try {
        const tarih = req.query.tarih || '2025-06-15';
        const saat = req.query.saat || '10:00:00';
        const formattedSaat = saat.length === 5 ? saat + ':00' : saat;
        ...
        `;
        const [rows] = await db.execute(queryStr, [tarih, tarih, formattedSaat]);
        return res.json(rows);
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: 'Server error' });
